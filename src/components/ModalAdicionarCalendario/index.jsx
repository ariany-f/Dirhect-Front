...
import styles from './ModalAdicionarCalendario.module.css';
...
function ModalAdicionarCalendario({ opened = false, calendario, aoFechar, aoSalvar }) {
    ...
    useEffect(() => {
        if (calendario && opened) {
            setIdOrigem(calendario.id_origem || '');
            setNome(calendario.nome || '');
            setDescricao(calendario.descricao || '');
        } else if (opened) {
            // Limpa os campos quando está adicionando
            setIdOrigem('');
            setNome('');
            setDescricao('');
            setClassError([]);
    ...
    const validarESalvar = () => {
        ...
        // Se tem calendario, está editando, senão está adicionando
        if (calendario) {
            aoSalvar(dadosParaAPI, calendario.id);
        } else {
                    ...
                    <DialogEstilizado open={opened}>
                        <Frame>
                            <Titulo>
                                ...
                                <h6>{calendario ? 'Editar Calendário' : 'Novo Calendário'}</h6>
                            </Titulo>
                        </Frame>
                        ...
                        <Wrapper>
                            ...
                            <Botao
                                ...
                                filled
                            >
                                {calendario ? 'Salvar Alterações' : 'Salvar Calendário'}
                            </Botao>
                        </Wrapper>
                    </DialogEstilizado>
                </Overlay>
...
export default ModalAdicionarCalendario;