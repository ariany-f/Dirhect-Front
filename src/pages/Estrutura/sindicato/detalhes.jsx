...
function SindicatoDetalhes() {
    ...
    useEffect(() => {
        if(!departamento) {
            http.get(`sindicato/${id}/?format=json`)
            .then(response => {
    ...
    const excluirDepartamento = () => {
        confirmDialog({
            message: 'Você quer excluir esse sindicato?',
            header: 'Deletar',
            icon: 'pi pi-info-circle',
            accept: () => {
                setLoading(true)
                http.delete(`api/sindicato/destroy/${id}`)
                .then(response => {
                   if(response.success)
                    {
    ...
    return (
        <ConteudoFrame>
            ...
            <Texto weight={500} size="12px">Sindicato</Texto>
            ...
            <BotaoGrupo align="space-between">
                <BotaoGrupo>
                    <Link to={`/estrutura/sindicato/detalhes/${id}/adicionar-colaboradores`}>
                        <Botao estilo={location.pathname == `/estrutura/sindicato/detalhes/${id}/adicionar-colaboradores`?'black':''} size="small" tab>Colaboradores</Botao>
                    </Link>
                    <Link to={`/estrutura/sindicato/detalhes/${id}`}>
                        <Botao estilo={location.pathname == `/estrutura/sindicato/detalhes/${id}`?'black':''} size="small" tab>Configuração de Benefícios</Botao>
                    </Link>
                </BotaoGrupo>
                
                {location.pathname == `/estrutura/sindicato/detalhes/${id}/adicionar-colaboradores` &&
                   <></>
                }
...
export default SindicatoDetalhes