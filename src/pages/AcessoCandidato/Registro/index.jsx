...
const AcessoCandidatoRegistro = ({ candidatoData, token }) => {
    const [candidato, setCandidato] = useState(candidatoData?.dados_candidato || {});
    const [admissao, setAdmissao] = useState(candidatoData || {});
    const [vaga, setVaga] = useState(candidatoData?.dados_vaga || {});
    ...
    const handleSalvarAdmissao = async () => {
        try {
            ...
            const response = await http.put(`candidato/atualizar-via-token/${token}/`, dadosParaEnviar);
    ...
    return (
        <ConteudoFrame>
            ...
            <div style={{ paddingBottom: '20px' }}> {/* Espaço para o footer fixo */}
                <Stepper 
                    headerPosition="top" 
                    ref={stepperRef} 
