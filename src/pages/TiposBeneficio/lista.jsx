...
import styles from './TiposBeneficio.module.css'
import { FaMapPin } from 'react-icons/fa'
import { Link } from 'react-router-dom'
import DataTableTiposBeneficio from '@components/DataTableTiposBeneficio'
...
function TiposBeneficioLista() {
    ...
    const loadBeneficios = () => {
        setLoading(true)
        let url = `tipo_beneficio/?format=json&page=${lazyParams.page}&page_size=${lazyParams.rows}`;
        ...
        http.post('tipo_beneficio/', data)
            .then(response => {
                if(response.id) {
                    ...
                    toast.current.show({
                        severity: 'success',
                        summary: 'Sucesso',
                        detail: 'Tipo de Benefício adicionado com sucesso',
                        life: 3000
                    });
                }
            })
            .catch(erro => {
                toast.current.show({
                    severity: 'error',
                    summary: 'Erro',
                    detail: 'Tipo deBenefício não pôde ser adicionado',
                    life: 3000
                });
            })
            .finally(function() {
        ...
        <>
            ...
            <ConteudoFrame>
                ...
                <DataTableTiposBeneficio 
                    ...
                    onSort={onSort}
                />
                <ModalTipoBeneficio aoSalvar={adicionarBeneficio} aoFechar={() => setModalOpened(false)} opened={modalOpened} />
            </ConteudoFrame>
        </>
    )
}

export default TiposBeneficioLista